...
class MBartAdapter(nn.Module):
    def __init__(self,input_dim, scale=0.0625):
        super(MBartAdapter,self).__init__()
        hidden_dim = int(input_dim*scale)
...
class MBartAdapterEncoderLayer(nn.Module):
    def __init__(self, config: MBartConfig):
        ...
        self.attention_adapter = MBartAdapter(self.embed_dim)
        self.output_adapter = MBartAdapter(self.embed_dim)
...
class MBartAdapterDecoderLayer(nn.Module):
    def __init__(self, config: MBartConfig):
        super().__init__()
        self.embed_dim = config.d_model
        ...
        self.attention_adapter = MBartAdapter(self.embed_dim)
        self.cross_attention_adapter = MBartAdapter(self.embed_dim)
        self.output_adapter = MBartAdapter(self.embed_dim)
    ...
    def forward(
        ...
        if encoder_hidden_states is not None:
            ...
            hidden_states = nn.functional.dropout(hidden_states, p=self.dropout, training=self.training)
            hidden_states = self.cross_attention_adapter(hidden_states)
            hidden_states = residual + hidden_states
...
class MBartAdapterPreTrainedModel(PreTrainedModel):
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, (MBartAdapterDecoder, MBartAdapterDecoder)):
...
class MBartAdapterEncoder(MBartAdapterPreTrainedModel):
    """
    ...
    def __init__(self, config: MBartConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        self.layers = nn.ModuleList([MBartAdapterEncoderLayer(config) for _ in range(config.encoder_layers)])
...
class MBartAdapterDecoder(MBartAdapterPreTrainedModel):
    """
    ...
    def __init__(self, config: MBartConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        self.layers = nn.ModuleList([MBartAdapterDecoderLayer(config) for _ in range(config.decoder_layers)])
...
class MBartAdapterModel(MBartAdapterPreTrainedModel):
    ...
    def __init__(self, config: MBartConfig):
        ...
        self.encoder = MBartAdapterEncoder(config, self.shared)
        self.decoder = MBartAdapterDecoder(config, self.shared)
...
class MBartAdapterForConditionalGeneration(MBartAdapterPreTrainedModel):
    base_model_prefix = "model"
    ...
    def __init__(self, config: MBartConfig):
        super().__init__(config)
        self.model = MBartAdapterModel(config)
